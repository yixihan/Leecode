package com.yixihan.month06.day0330;
...
public class Solution {
    public List<Integer> busiestServers(int k, int[] arrival, int[] load) {
        ...
        while (index < n) {
            // 服务器编号
            int serverNo = index % k;
            // 任务开始时间
            int startTime = arrival[index];
            ...
            Integer nextNo = freeServer.ceiling (serverNo);
            ...
            // 编号为 idx 的任务的结束时间
            int endTime = arrival[index] + load[index];

            // 将 <新任务结束时间, 服务器> 加入队列
            queue.add (new int[]{endTime, nextNo});
            index++;
        }

        while (!queue.isEmpty ()) {
            int serverNo = queue.poll ()[1];
            serverDeals[serverNo]++;
        }
