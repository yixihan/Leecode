...
public class StockPrice {
    ...
    public void update(int timestamp, int price) {
        if (map.containsKey (timestamp)) {
            treeMap.put (map.get (timestamp), treeMap.getOrDefault (map.get (timestamp), 0) - 1);
...
//        return map.values ().stream ().mapToInt (e -> e).max ().orElse (Integer.MIN_VALUE);
    }

    public int minimum() {
        return treeMap.firstKey ();
//        return map.values ().stream ().mapToInt (e -> e).min ().orElse (Integer.MAX_VALUE);
    }
}
